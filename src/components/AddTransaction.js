...
const AddTransaction = ({ addTransaction, currencySymbol , prevStep , categories , updateCategories }) => {
  ...
  const onSubmit = async (e) => {
    ...
    const response = await fetch(API_GET_ACCOUNT + 
      '?account_id=' + accountRequestBody.accountId +
      '&user_id=' + accountRequestBody.userId,
        {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
    ...
    switch (currentWindow) {
      case 1:
        return (
          ...
          <button class="back-button"
              ...
              <div>
              <h1 className="mainHeader" style={{marginLeft:'16px'}}>Add transaction</h1>
              </div>
              ...
              <form onSubmit={onSubmit}>
                ...
                <ListButton text="Category" onClick={showCategories} 
                data = {category}></ListButton>

                </div>

                <button  
                className={!account && !category ? "DisabledLoginBubbleButton" : "LoginBubbleButton"} 
                type="submit"
      ...
      case 2:
        return (
          ...
          <button class="back-button"
          onClick={showAddTransactionWindow}>
              <img
              src={BackButtonIcon} // URL вашей картинки
              alt="icon"
              style={{ }}
            ...
            <ul className="list">
              {categories.map((item) => (
                <div>
                  ...
                  <div className='categoryHeaderMain'>{item.category} </div>  
                  <div className='ImgBox'>
                  <img
                        src={AngleIcon} 
                        alt="icon"
